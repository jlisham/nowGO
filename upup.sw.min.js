...
var _parseSettingsAndCache = function(settings) {
  var newCacheName =
    ...
    _calculateHash(
      settings["content"] + settings["content-url"] + settings["assets"]
    );
  return caches
    .open(newCacheName)
    .then(function(cache) {
      ...
      if (settings["content-url"]) {
        return fetch(settings["content-url"], { mode: "no-cors" }).then(
          function(response) {
        ...
        return cache.put(
          "sw-offline-content",
          _buildResponse(settings.content)
        );
        // store default content
      } else {
        return cache.put(
          "sw-offline-content",
          _buildResponse("You are offline")
        );
      }
    })
    .then(function() {
      ...
      return caches.keys().then(function(cacheNames) {
        return Promise.all(
          cacheNames.map(function(cacheName) {
            if (
              cacheName.startsWith(_CACHE_NAME_PREFIX) &&
